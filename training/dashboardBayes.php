    ...
    <table class="table table-striped table-bordered data">
      <thead>
        <tr>
          ...
          <th>BOBOT NB POSITIF</th>
          <th>BOBOT NB NEGATIF</th>
          <th>BOBOT NB NETRAL</th>
        </tr>
...
<?php 
	...
	<!-- PROSES BOBOT NB-->
<button style="float: left; margin-top:0px;" type="submit" name="bobotbayes" value="BOBOTBAYES" class="btn btn-primary">PROSES NAIVE BAYES</button>
<?php
    if(isset($_POST['bobotbayes'])){
    $NBc=new functNB();
    //$NBc->bobotbayestraining($konek);
    $NBc->set_probabilitas_kata_positif($konek);
    $NBc->set_probabilitas_kata_negatif($konek);
		$NBc->set_probabilitas_kata_netral($konek);


		?>
                                        <script type="text/javascript">
                                            alert('Data berhasil diberi Bobot Naive Bayes');
                                            document.location.href="dashboardBayes.php";
                                        </script>
                                        <?php
	} 
