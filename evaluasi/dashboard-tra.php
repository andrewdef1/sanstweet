...
?>
  ...
  $query=mysqli_query($konek, "SELECT id_tweet FROM tbltweet WHERE predicted='positive';");
  $pos=mysqli_num_rows($query);
  $query=mysqli_query($konek, "SELECT id_tweet FROM tbltweet WHERE predicted='negative';");
  $neg=mysqli_num_rows($query);
  $query=mysqli_query($konek, "SELECT id_tweet FROM tbltweet WHERE predicted='neutral';");
...
<div class="row">
   ...
   <div class="col-md-4 mb-2">
		...
		var myChart = new Chart(ctx, {
			...
			data: {
				...
				datasets: [{
					...
					data: [
					<?php 
					$jumlah_pos = mysqli_query($konek,"SELECT predicted FROM tbltweet WHERE predicted='positive' ");
					echo mysqli_num_rows($jumlah_pos);
					?>, 
					<?php 
					$jumlah_neg = mysqli_query($konek,"SELECT predicted FROM tbltweet WHERE predicted='negative' ");
					echo mysqli_num_rows($jumlah_neg);
					?>, 
					<?php 
					$jumlah_net = mysqli_query($konek,"SELECT predicted FROM tbltweet WHERE predicted='neutral'");
	...
	<?php
			//TP
			$sql = mysqli_query($konek, "SELECT count(*) as total FROM tbltweet WHERE kategori = 'positive' AND predicted='positive'");
    		$tpCount = mysqli_fetch_assoc($sql);
    		$tpCount = $tpCount['total'];
        echo "TP".$tpCount."<br>";
			
			//TN
    		$sql = mysqli_query($konek, "SELECT count(*) as total FROM tbltweet WHERE kategori = 'negative' AND predicted='negative'");
    		$tnCount = mysqli_fetch_assoc($sql);
    		$tnCount = $tnCount['total'];
echo "TN".$tnCount."<br>";
    		//TNet
    		$sql = mysqli_query($konek, "SELECT count(*) as total FROM tbltweet WHERE kategori = 'neutral' AND predicted='neutral'");
    		$tnetCount = mysqli_fetch_assoc($sql);
    		$tnetCount = $tnetCount['total'];
			echo "TNet".$tnetCount."<br>";
			//FP
    		$sql = mysqli_query($konek, "SELECT count(*) as total FROM tbltweet WHERE kategori = 'positive' AND predicted NOT LIKE '%positive%'");
    		$fpCount = mysqli_fetch_assoc($sql);
    		$fpCount = $fpCount['total'];
			echo "FP".$fpCount."<br>";
			//FN
			$sql = mysqli_query($konek, "SELECT count(*) as total FROM tbltweet WHERE kategori = 'negative' AND predicted NOT LIKE '%negative%'");
    		$fnCount = mysqli_fetch_assoc($sql);
    		$fnCount = $fnCount['total'];
    					echo "FN".$fnCount."<br>";
			//FNet
			$sql = mysqli_query($konek, "SELECT count(*) as total FROM tbltweet WHERE kategori = 'neutral' AND predicted NOT LIKE '%neutral%'  ");
